    ...
    class Giay
    {
        ...
        public void LoadDanhSach(string idLoaiGiay)
        {
            ...
            else
            {
                str = "select * from HANGGIAY, LOAIGIAY, GIAY  where " +
                    "GIAY.status=1 and GIAY.idLoaiGiay = LOAIGIAY.idLoaiGiay and LOAIGIAY.idHangGiay = HANGGIAY.idHangGiay" +
                    " and LOAIGIAY.idLoaiGiay='"+ idLoaiGiay + "'";
        ...
        public string CapNhat(int index, string soluong, string giaban)
        {
            string str = "update GIAY set soLuongGiay='" + soluong + "', giaBan = '"+giaban+"' where status = 1 and idGiay = " + giay_tb.Rows[index]["idGiay"].ToString();
